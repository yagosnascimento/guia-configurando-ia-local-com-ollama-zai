# Guia Definitivo: Configurando sua IA Local com Ollama, DeepSeek Coder e VS Code
...
3. [Passo 2: Baixando e Executando o DeepSeek Coder](#3-passo-2-baixando-e-executando-o-deepseek-coder)
...
Rodar modelos de IA localmente exige recursos específicos. Verifique se seu sistema atende aos requisitos mínimos para o **DeepSeek Coder**:
...
## 3. Passo 2: Baixando e Executando o DeepSeek Coder

A DeepSeek desenvolve modelos especializados em código com excelente precisão para refatoração e compreensão de diversas linguagens.
...
5.  Adicione a configuração do modelo na seção `models` (substitua a configuração anterior se existir):
    ```json
    {
      "models": [
        {
...
* **Modelos Maiores:** Se você tiver mais de 16GB de VRAM/RAM, pode testar versões maiores para maior precisão, como `ollama run deepseek-coder:33b`.
...
with open("README_IA_Local.md", "w", encoding="utf-8") as f:
    f.write(content)

```
Excelente escolha! O `deepseek-coder` é fantástico para isso.

Atualizei o conteúdo do arquivo `README_IA_Local.md` substituindo todas as referências do GLM pelo **DeepSeek Coder**, ajustando também a configuração JSON para refletir a mudança no VS Code.

[file-tag: code-generated-file-0-1776948765983641248]

Aqui está o conteúdo atualizado para você conferir:
